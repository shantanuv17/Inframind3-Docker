...
public class RegistrationDAO {
    ...
    static
    {
        try
        {
            ps1=DBdata.getConnection().prepareStatement("select * from members where username=? and membertype=?");
            ps2=DBdata.getConnection().prepareStatement("insert into members values(?,?,?)");
        }
        catch(Exception ex)
        {
